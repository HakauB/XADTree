...
/**
 * <!-- globalinfo-start --> Class for generating an alternating decision tree.
 * The basic algorithm is based on:<br/>
 * <br/>
 ...
 * Bled, Slovenia, 124-133, 1999.<br/>
 * <br/>
 * This version currently only supports two-class problems. The number of
 * boosting iterations needs to be manually tuned to suit the dataset and the
 * desired complexity/accuracy tradeoff. Induction of the trees has been
 * optimized, and heuristic search methods have been introduced to speed
 * learning.
 * <p/>
 * <!-- globalinfo-end -->
 *
 ...
 */
public class ADTree extends AbstractClassifier implements OptionHandler, Drawable, AdditionalMeasureProducer,
		WeightedInstancesHandler, IterativeClassifier, TechnicalInformationHandler {
	...
	/** The search modes */
	public static final Tag[] TAGS_SEARCHPATH = { new Tag(SEARCHPATH_ALL, "Expand all paths"),
	...
	/**
	 * The training instances with positive class - referencing the training
	 * dataset
	 */
	protected ReferenceInstances m_posTrainInstances;

	/**
	 * The training instances with negative class - referencing the training
	 * dataset
	 */
	 ...
	 * Will add a new splitter node and two prediction nodes to the tree (unless
	 * merging takes place).
	 *
	 * @exception Exception
	 *                if try to boost without setting up tree first or there are
	 *                no instances to train with
	 */
	 ...
	 * aiming to minimize the Z value.
	 *
	 * @exception Exception
	 *                if search fails
	 */
	...
	/**
	 * Recursive function that carries out search for the best test (splitter)
	 * to add to this part of the tree, by aiming to minimize the Z value.
	 * Performs Z-pure cutoff to reduce search space.
	 *
	 * @param currentNode
	 *            the root of the subtree to be searched, and the current node
	 *            being considered as parent of a new split
	 * @param posInstances
	 *            the positive-class instances that apply at this node
	 * @param negInstances
	...
	/**
	 * Continues single (two-class optimized) search by investigating every node
	 * in the subtree under currentNode.
	 *
	 * @param currentNode
	...
	/**
	 * Continues single (two-class optimized) search by investigating only the
	 * path with the most heavily weighted instances.
	 *
	 * @param currentNode
	 *            the root of the subtree to be searched
	 * @param posInstances
	...
	private void goDownHeaviestPathSingle(PredictionNode currentNode, Instances posInstances, Instances negInstances)
		...
		for (Enumeration e = currentNode.children(); e.hasMoreElements();) {
			Splitter split = (Splitter) e.nextElement();
			for (int i = 0; i < split.getNumOfBranches(); i++) {
				double weight = split.instancesDownBranch(i, posInstances).sumOfWeights()
		...
		if (heaviestSplit != null)
			searchForBestTestSingle(heaviestSplit.getChildForBranch(heaviestBranch),
					heaviestSplit.instancesDownBranch(heaviestBranch, posInstances),
					heaviestSplit.instancesDownBranch(heaviestBranch, negInstances));
	}

	/**
	 * Continues single (two-class optimized) search by investigating only the
	 * path with the best Z-pure value at each branch.
	 *
	 * @param currentNode
	 *            the root of the subtree to be searched
	 * @param posInstances
	...
	/**
	 * Continues single (two-class optimized) search by investigating a random
	 * path.
	 *
	 * @param currentNode
	 *            the root of the subtree to be searched
	 ...
	 * If it finds a split that has a Z-value lower than has already been found
	 * it will update the search information to record this as the best option
	 * so far.
	 *
	 * @param attIndex
	 *            index of a nominal attribute to create a split from
	 * @param currentNode
	 *            the parent under which a split is to be considered
	 * @param posInstances
	 *            the positive-class instances that apply at this node
	 * @param negInstances
	 *            the negative-class instances that apply at this node
	 */
	private void evaluateNominalSplitSingle(int attIndex, PredictionNode currentNode, Instances posInstances,
	 ...
	 * currentNode. If it finds a split that has a Z-value lower than has
	 * already been found it will update the search information to record this
	 * as the best option so far.
	 *
	 * @param attIndex
	 *            index of a numeric attribute to create a split from
	 * @param currentNode
	 *            the parent under which a split is to be considered
	 * @param posInstances
	 *            the positive-class instances that apply at this node
	...
	/**
	 * Updates the weights of instances that are influenced by a new prediction
	 * value.
	 *
	 * @param posInstances
	 *            positive-class instances to which the prediction value applies
	 * @param negInstances
	 *            negative-class instances to which the prediction value applies
	 * @param predictionValue
	 *            the new prediction value
	 */
	...
	/**
	 * Finds the nominal attribute value to split on that results in the lowest
	 * Z-value.
	 *
	 * @param posInstances
	 *            the positive-class instances to split
	 * @param negInstances
	 *            the negative-class instances to split
	 * @param attIndex
	 *            the index of the nominal attribute to find a split for
	 * @return a double array, index[0] contains the value to split on, index[1]
	 *         contains the Z-value of the split
	 */
	private double[] findLowestZNominalSplit(Instances posInstances, Instances negInstances, int attIndex) {
		...
		if (maxIndex == 2)
			maxIndex = 1; // avoid repeating due to 2-way symmetry
	 ...
	 *
	 * @param instances
	 *            the instances to get the weights from
	 * @param attIndex
	...
	private double[] attributeValueWeights(Instances instances, int attIndex) {
		...
		for (Enumeration e = instances.enumerateInstances(); e.hasMoreElements();) {
			...
			if (!inst.isMissing(attIndex))
				weights[(int) inst.value(attIndex)] += inst.weight();
		}
		return weights;
	}
	 ...
	 *
	 * @param instances
	 *            the instances to find a split for
	 * @param attIndex
	 *            the index of the numeric attribute to find a split for
	 * @return a double array, index[0] contains the split-point, index[1]
	 *         contains the Z-value of the split
	 * @throws Exception
	 *             in case of an error
	 */
	 ...
	 *            the instance
	 * @param currentNode
	 *            the root of the tree to get the values from
	 * @param currentValue
	 ...
	 *
	 * @param currentNode
	 *            the current node under investigation
	 * @param level
	 ...
	 *
	 * @param currentNode
	 *            the currentNode under investigation
	 * @param text
	 *            the string built so far
	 * @param splitOrder
	 *            the order the parent splitter was added to the tree
	 * @param predOrder
	...
	protected void graphTraverse(PredictionNode currentNode, StringBuffer text, int splitOrder, int predOrder,
		...
		for (Enumeration e = currentNode.children(); e.hasMoreElements();) {
			...
			for (int i = 0; i < split.getNumOfBranches(); i++) {
				...
				if (child != null) {
					text.append("S" + split.orderAdded + "->" + "S" + split.orderAdded + "P" + i + " [label=\""
							+ split.comparisonString(i, m_trainInstances) + "\"]\n");
	 ...
	 *
	 * -E num <br>
	 * Set the nodes to expand: -3(all), -2(weight), -1(z_pure), >=0 seed for
	 * random walk (default -3)
	 * <p>
	 *
	 * -D <br>
	 * Save the instance data with the model
	...
	public void setOptions(String[] options) throws Exception {
		...
		if (eString.length() != 0) {
			...
			if (value >= 0) {
				...
				setSearchPath(new SelectedTag(value + 3, TAGS_SEARCHPATH));
		}
	...
	/**
	 * Returns the next number in the order that splitter nodes have been added
	 * to the tree, and records that a new splitter has been added.
	 *
	...
	/**
	 * Frees memory that is no longer needed for a final model - will no longer
	 * be able to increment the classifier after calling this.
	 *
	 */
	public void done() {
	...
	/**
	 * Creates a clone that is identical to the current tree, but is
	 * independent. Deep copies the essential elements such as the tree nodes,
	 * and the instances (because the weights change.) Reference copies several
	 * elements such as the potential splitter sets, assuming that such elements
	 * should never differ between clones.
	 *
	 * @return the clone
	 */
	public Object clone() {
		...
		if (m_root != null) { // check for initialization first
			clone.m_root = (PredictionNode) m_root.clone(); // deep copy the
															// tree

			clone.m_trainInstances = new Instances(m_trainInstances); // copy
																		// training
																		// instances
			...
			if (m_random != null) {
				...
				} // we know that Random is serializable
			...
			if (m_posTrainInstances != null) {
				clone.m_posTrainInstances = new ReferenceInstances(m_trainInstances,
						m_posTrainInstances.numInstances());
				clone.m_negTrainInstances = new ReferenceInstances(m_trainInstances,
						m_negTrainInstances.numInstances());
				for (Enumeration e = clone.m_trainInstances.enumerateInstances(); e.hasMoreElements();) {
					Instance inst = (Instance) e.nextElement();
					try { // ignore classValue() exception
						if ((int) inst.classValue() == 0)
							clone.m_negTrainInstances.addReference(inst); // belongs
																			// in
																			// negative
																			// class
						else
							clone.m_posTrainInstances.addReference(inst); // belongs
																			// in
																			// positive
																			// class
					} catch (Exception ignored) {
	 ...
	 * training instances are compatible - strange things could occur if they
	 * are not.
	 *
	 * @param mergeWith
	...
	@Override
	public void initializeClassifier(Instances arg0) throws Exception {
